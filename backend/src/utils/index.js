...
export function parseTime(dateString, cFormat) {
  ...
  try {
    ...
    if (typeof dateString === 'object') {
      ...
      if (('' + dateString).length === 10) dateString = parseInt(time) * 1000
      date = new Date(dateString)
    }
    if (isNaN(date))
    {
      dateString = dateString.replace(/-/g, '/').replace(/T/g, ' ').replace(/\..*/g,'');
